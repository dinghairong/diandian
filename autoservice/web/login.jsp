...
<body style="min-width: 1280px;">
	<div class="all" id="all">
		...
		<div class="login_div" style="margin: 61px 226px 0 0;">
			<form id="member_login_form" onsubmit="return validateLogin();" action="<%=request.getContextPath()%>/former/login.do" method="post" style="box-sizing: unset;">
				<div class="login-form-row" style="margin-top: 26px;">
					<div class="title">账号：</div>
					<div class="input">
						<input id="login_name_input" class="pub-input" type="text" name="account"  placeholder="请输入您的个人帐号">
					</div>
				</div>
				...
				<div class="login-form-row">
					...
					<div class="input">
						<input id="login_password_input" class="pub-input" type="password" name="password" placeholder="请输入您的登录密码">
					</div>
				</div>
			...
			<img src="<%=ctx%>/images/login_new.png" 
	...
	<input type="hidden" id="history_url" value="<%=historyurl%>">
</div>
...
function PassWordRecv(){
	...
	if(phone.length == 0){
		...
		$.ajax({
			url: "<%=ctx%>/former/checkphone.do?phone=" + phone + "&imagecode=" + imgcode,
			type:"POST",
			dataType: "JSON",
			async: false,
			timeout: 3000,
...
function loginHX(){
	$.ajax({
		url: "<%=request.getContextPath()%>/former/login.do",
		type: "post",
		dataType: "JSON",
