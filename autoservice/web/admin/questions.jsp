...
<![endif]-->
		...
		jQuery(function($) {
			...
			jQuery(grid_selector).jqGrid({
				//direction: "rtl",
				url: "<%=ctx%>/admin/getQuestionList.do",
				//data: grid_data,
				datatype: "json",
				height: 391,
				colNames:['ID','客户名称','客户问题','提问时间','管理员名称','答复内容','答复时间','常见问题'],
				colModel:[
					{name:'id',index:'id',hidden:true,editable:false},
					{name:'clientName',index:'a.user1.name',editable:true,edittype:"text",editoptions:{readonly:true}},
					{name:'ask',index:'a.usecontent',search:false,sortable:false,editable:true,edittype:"textarea",editoptions:{readonly:true,rows:"4",cols:"50"}},
					{name:'askTime',index:'a.usetime',search:false,editable:true,edittype:"text",editoptions:{readonly:true}},
					{name:'adminName',index:'a.user2.name',editable:true,edittype:"text",editoptions:{readonly:true}},
					{name:'adv.uucontent',index:'a.uucontent',search:false,sortable:false,editable:true,edittype:"textarea",editrules:{required:true},editoptions:{rows:"4",cols:"50"}},
					{name:'replyTime',index:'a.uutime',search:false,editable:true,edittype:"text",editoptions:{readonly:true}},
					{name:'adv.iscommon',index:'a.iscommon',editable:true,edittype:"select",editoptions:{value:"0:是;1:否"}},
				], 
				...
				},
				{
					...
					serializeEditData: function(data) {
						...
						for (key in data) {
							...
							if (key == "id"){
								postdata +="adv.id="+jQuery(grid_selector).getRowData(data[key]).id;
							} else if (key == "clientName" || key == "ask" || key == "askTime"
									|| key == "replyTime" || key == "adminName") {
								continue;
							} else {
								postdata +=key+"="+data[key];
							}
					...
					onclickSubmit: function(params,postdata) {
						params.url = "<%=ctx%>/admin/updateQuestion.do";
						alert(编辑成功);
					}
				},
				{
					...
					width: 750,
					mtype: "POST",
					closeAfterAdd: true, // special
					recreateForm: true,
					beforeShowForm : function(e) {
						var form = $(e[0]);
						form.closest('.ui-jqdialog').find('.ui-jqdialog-titlebar').wrapInner('<div class="widget-header" />');
						style_edit_form(form);
						jQuery("tr#tr_ask textarea:eq(0)").removeAttr("readonly");
					},
					afterShowForm: function(form) {
						form.css("height", "455px");
					},
					serializeEditData: function(data) {
						...
						for (key in data) {
							...
							if (key == "clientName" || key == "askTime"
									|| key == "replyTime" || key == "adminName") {
								continue;
							} else if (key == "ask") {
								postdata += "adv.usecontent="+data[key];
							} else {
								postdata +=key+"="+data[key];
							}
						}
					...
					onclickSubmit: function(params,postdata) {
						params.url = "<%=ctx%>/admin/addQuestion.do";
						alert("添加成功");
					}
				},
				{
					...
					onclickSubmit: function(rp_ge, postdata) {
						rp_ge.url = "<%=ctx%>/admin/removeQuestion.do";
					},
