		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					//direction: "rtl",
					url: "<%=ctx%>/admin/sysUserList.do",
					//data: grid_data,
					datatype: "json",
					height: 391,
					colNames:['ID','用户名','密码','角色','手机号码','邮箱','省','市','区','头像','头像路径','状态'],
					colModel:[
						{name:'adminUser.id',index:'adminUser.id',hidden:true,editable:false},
						{name:'adminUser.name',index:'u.name',editable:true,edittype:"text",editoptions:{readonly:true}},
						{name:'adminUser.password',index:'u.password',hidden:true,search:false,editable:true,edittype:"password",editrules:{edithidden:true},editoptions:{maxlength:15}},
						{name:'adminUser.role.id',index:'u.role.displayName',editable:true,edittype:"select",editoptions:{value:"<%=roles%>"}},
						{name:'adminUser.phone',index:'u.phone',editable:true,edittype:"text",editrules:{required:true,integer:true}},
						{name:'adminUser.email',index:'u.email',editable:true,edittype:"text",editrules:{required:true,email:true}},
						{name:'province',index:'province',editable:true,edittype:"select",editoptions:{value:"<%=provinceBuf.toString()%>"}},
						{name:'city',index:'city',editable:true,edittype:'text',editrules:{readonly:true,required:true}},
						{name:'county',index:'county',editable:true,edittype:'text',editrules:{readonly:true,required:true}},
						...
						{name:'adminUser.status',index:'u.status',search:false,hidden:true,editable:false},
					], 
				...
				jQuery(grid_selector).jqGrid('navGrid',pager_selector,
					...
					},
					{
						...
						serializeEditData: function(data) {
							...
							for (key in data) {
								...
								if (key == "iconPN") {
									...
									postdata +="adminUser.id"+"="+jQuery(grid_selector).getRowData(data[key])['adminUser.id']+"&idx="+data[key];
								} else {
									postdata +=key+"="+data[key];
								}
							}
						...
						onclickSubmit: function(params,postdata) {
							params.url = "<%=ctx%>/admin/updateSysUser.do?";
						},
						afterSubmit: function(response, postdata) {
							...
							if (data.success == true && $('#icon').val() != '') {
								...
								$.ajaxFileUpload(
									{
										url: "<%=ctx%>/admin/uploadAdminUserAvatar.do",
										secureuri: false,
										fileElementId: 'icon',
										dataType: 'json',
										data: {id:data.id, "adminUser.id":data.id},
										success: function (data, status) {
											if (typeof (data.success) != 'undefined') {
												if (data.success == true) {
													alert("修改成功"),
													/* if (data.id == jQuery(grid_selector).getRowData(parseInt(data.idx))['admiUser.id']) {
														// update iconPN column value
														jQuery(grid_selector).setRowData(parseInt(data.idx), data.data);
													} */
					...
					},
					{
						...
						beforeShowForm : function(e) {
							...
							style_edit_form(form);
							show_icon(form);
							jQuery("tr#tr_adminUser\\.name input:eq(0)", form).removeAttr("readonly");
						...
						onclickSubmit: function(params, postdata) {
							params.url = "<%=ctx%>/admin/addSysUser.do";
						},
						afterSubmit: function(response, postdata) {
							var data = $.parseJSON(response.responseText);
							if (data.success == true && $('#icon').val() == ''){
					...
					},
					{
						...
						onclickSubmit: function(rp_ge, postdata) {
							rp_ge.url = "<%=ctx%>/admin/removeUser.do";
						},
