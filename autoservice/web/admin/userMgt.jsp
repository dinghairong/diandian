...
String roles = "";
List<Role> roleList = roleDao.getBackendAdminRoles();
for (int i=0; i<roleList.size();i++) {
	roles+= roleList.get(i).getId() + ":" + roleList.get(i).getDisplayName();
...
/* RatingDao ratDao = (RatingDao)SpringUtils.getBean(RatingDao.class);
String ratings = "";
List<Rating> ratList = ratDao.getAll();
for (int i=0;i<ratList.size(); i++) {
	ratings += ratList.get(i).getId() + ":" + ratList.get(i).getName();
	if (i != (ratList.size() - 1)) {
		ratings +=";";
	}
} */
		...
		<script type="text/javascript">
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					//direction: "rtl",
					url: "<%=ctx%>/admin/getUserList.do",
					//data: grid_data,
					datatype: "json",
					height: 391,
					colNames:['ID','用户名','密码','手机号码','邮箱','省市县(区)','省份','城市','县区','余额','头像','头像路径','是否有效','创建时间'],
					colModel:[
						{name:'uid',index:'uid',hidden:true,editable:false},
						{name:'user.name',index:'u.name',editable:true,edittype:"text"},
						{name:'user.password',index:'u.password',hidden:true,search:false,editable:true,edittype:"password",editoptions:{maxlength:15},editrules:{edithidden:true}},
						{name:'user.phone',index:'u.phone',editable:true,edittype:"text",editrules:{required:true,edithidden:true,integer:true}},
						{name:'user.email',index:'u.email',editable:true,edittype:"text",editrules:{required:true,email:true}},
						{name:'user.cid',index:'u.cid',hidden:true,editable:true,search:false},
						...
						{name:'user.balance',index:'u.balances',search:false,editable:false,edittype:"text"},
						...
						{name:'user.status',index:'u.status',search:false,hidden:true,editable:false},
						{name:'user.creatTime',index:'user.creatTime',editable:false,search:false},
					], 
			
					viewrecords : true,//定义是否要显示总记录数
					rowNum:10,//在grid上显示记录条数，这个参数是要被传递到后台
					rowList:[10,20,30],//一个下拉选择框，用来改变显示记录数，当选择时会覆盖rowNum参数传递到后台
					sortname: "u.name",//排序列的名称，此参数会被传到后台
	                ...
	                prmNames:{ //Default valuesprmNames设置jqGrid将要向Server传递的参数名称
	                	rows:"page.pageSize", // 表示请求行数的参数名称 
	                	page:"page.curPageNo", // 表示请求页码的参数名称 
	                	sort:"page.orderBy", // 表示用于排序的列名的参数名称 
	                	order:"page.order", // 表示采用的排序方式的参数名称 
	                	search:"page._search", // 表示是否是搜索请求的参数名称 
	                	editoper:"page._edit",// 当在edit模式中提交数据时，操作的名称 
	                	addoper:"page._add",// 当在add模式中提交数据时，操作的名称 
	                	deloper:"page._delete"// 当在delete模式中提交数据时，操作的名称 
	                },
				...
				jQuery(grid_selector).jqGrid('navGrid',pager_selector,
					...
					},
					{
						...
						beforeShowForm : function(e) {
							...
							show_icon(form);
							$("#tr_user\\.phone").attr("readonly", "readonly");
						...
						serializeEditData: function(data) {
							...
							for (key in data) {
								...
								if (key == "iconPN") {
									...
									postdata +="user.id"+"="+jQuery(grid_selector).getRowData(data[key]).uid+"&idx="+data[key];
								} else {
									postdata +=key+"="+data[key];
								}
							}
						...
						onclickSubmit: function(params,postdata) {
							params.url = "<%=ctx%>/admin/updateUser.do";
						},
						afterSubmit: function(response, postdata) {
							...
							if (data.success == true && $('#icon').val() != '') {
								$.ajaxFileUpload(
									{
										url: "<%=ctx%>/admin/uploadUserAvatar.do",
										secureuri: false,
										fileElementId: 'icon',
										dataType: 'json',
										data: {id:data.id, "user.id":data.id},
					...
					{mtype: "POST",
						...
						onclickSubmit: function(rp_ge, postdata) {
							rp_ge.url = "<%=ctx%>/admin/removeSysUser.do";
						},
				...
				function show_icon(form) {
					...
					if (tmp == null || tmp[1] == "") {
						...
						src = "<%=ctx%>/admin/displayPic.do?imagePath="+tmp[0]+"&imageName="+tmp[1];
					}
