...
StringBuffer paidTypeStr = new StringBuffer();
PaidTypeDao ptDao = (PaidTypeDao)SpringUtils.getBean(PaidTypeDao.class);
List<PaidType> pList = ptDao.getAll();
for (PaidType p : pList) {
	paidTypeStr.append(p.getId() + ":" + p.getName());
	paidTypeStr.append(";");
}
if (paidTypeStr.length() > 0) {
	paidTypeStr.deleteCharAt(paidTypeStr.length() - 1);
}
...
<![endif]-->
		...
		<script type="text/javascript">
			...
			function ajax_upload_icon(fileIds, picType, which,form){
				if (which.files[0].size > 2097152) {
					alert("请选择小于2M的图片！");
					return;
				}
					...
					success: function(data){
						if(typeof (data.success) != 'undefined'){
							if(data.success == true) {
								...
								jQuery("#icon-img").attr("src",src);
								
								updateShopImgInfo("serviceshop_image");
							}else{
								alert(data.message);
							};
						}else{
			...
			function updateShopImgInfo(type){
				if(type== null || type!="serviceshop_image"){
					return;
				}
				var img = jQuery("#icon-img")[0];
				var imgval ="";
				var src= img.src;
				var start = src.indexOf("imagePath=");
				var end = src.indexOf("&imageName=");
			...
			function ajax_upload_licensePic(fileIds, picType, which,form){
				if (which.files[0].size > 2097152) {
					alert("请选择小于2M的图片！");
					return;
				}
					...
					success: function(data){
						if(typeof (data.success) != 'undefined'){
							if(data.success == true) {
								...
								jQuery("#license-img").attr("src",src);
								
								updateShoplicenseInfo("serviceshop_image");
							}else{
								alert(data.message);
							};
						}else{
			...
			function updateShoplicenseInfo(type){
				if(type== null || type!="serviceshop_image"){
					return;
				}
				var img = jQuery("#license-img")[0];
				var imgval ="";
				var src= img.src;
				var start = src.indexOf("imagePath=");
				var end = src.indexOf("&imageName=");
			...
			jQuery(function($) {
				...
				jQuery(grid_selector).jqGrid({
					//direction: "rtl",
					url: "<%=ctx%>/admin/shopList.do",
					//data: grid_data,
					datatype: "json",
					height: 391,//表格高度，可以是数字，像素值或者百分比
					<%--colNames:['ID','名称','经度','纬度','省市县(区)','省份','城市','县区','地址','服务项目','营业时间','付款方式',--%>
					colNames:['ID','名称','经度','纬度','省市县(区)','省份','城市','县区','地址','服务项目','营业时间',
					...
					colModel:[
						...
						{name:'service',index:'service',editable:true,edittype:"text",sortable:false,search:true},
						{name:'s.business',index:'s.business',sortable:false,search:false,editable:true,edittype:"text",editrules:{required:true}},
						<%--{name:'paidtype',index:'paidtype',editable:true,sortable:true,hidde:true,search:false,edittype:"select",editoptions:{value:"<%=paidTypeStr.toString()%>"}},--%>
						...
						{name:'s.years',index:'s.years',editable:true,search:false,edittype:"text",editrules:{required:true}},
						{name:'s.sizes',index:'s.sizes',editable:true,search:false,edittype:'text',editrule:{required:true}},
						{name:'s.people',index:'s.people',editable:true,search:false,edittype:'text',editrules:{required:true,integer:true}},
						{name:'s.status',index:'s.status',editable:true,edittype:'select',editoptions:{value:"0:审核通过;1:待审核;2:已删除;3:其它"}},
						{name:'s.updateTime',index:'s.updateTime',editable:false,search:false,hidden:false}
					], 
				...
				function getCoordinate() {
					...
					var addr =  jQuery("#province option:selected").val()  
						+ jQuery("#tr_city input:eq(0)").val() 
						+ jQuery("#tr_county input:eq(0)").val() 
						+ jQuery("#tr_s\\.address input:eq(0)").val();
				...
				jQuery(grid_selector).jqGrid('navGrid',pager_selector,
					...
					},
					{
						...
						beforeShowForm : function(e) {
							...
							$("#tr_service td:eq(1)").append("<div style=\"margin-top: -20px;\" class=\"widget-header header-color-blue2\">"
								+ "<h4 class=\"lighter smaller\">选择服务产品</h4></div><div id='serviceTree' class='tree'></div>");
						...
						serializeEditData: function(data) {
							var postdata="";
							var src = $("#icon-img")[0].src;
							var start = src.indexOf("imagePath=");
							var end = src.indexOf("&imageName=");
							
							var path = src.substring(start + 10, end);
							var icon = src.substring(end + 11);
							
							var lsrc = $("#license-img")[0].src
							
							var start = lsrc.indexOf("imagePath=");
							var end = lsrc.indexOf("&imageName=");
							
							
							var license = lsrc.substring(end + 11);
							...
							for (key in data) {
								...
								if (key == "id"){
									postdata +="s.id="+jQuery(grid_selector).getRowData(data[key])['s.id']+"&idx="+data[key];
								} else if (key == 'service') {
									// set services
									var services = "";
									...
									postdata += "s.path="+path;
																} else {
									postdata += key+"="+data[key];
								}
							}
							postdata += "&s.image="+icon+"&s.license="+license;
							return postdata;
						},
					...
					},
					{
						...
						beforeShowForm : function(e) {
							...
							show_thumbnails(form);
							style_edit_form(form);
							$("tr#tr_s\\.phone input:eq(0)", form).removeAttr("readonly");
							$("#tr_s\\.address input:eq(0)").blur(getCoordinate);
							...
							selTime += "<div class=\"input-append\"><label for='endTime'>结束时间</label>&nbsp;&nbsp;<input type='text' id='endTime' size='4'></div>";
							$("#tr_s\\.business td:eq(1)").append($(selTime));
							$("#startTime").timepicker({showMeridian:false,defaultTime:"08:00"});
							$("#endTime").timepicker({showMeridian:false,defaultTime:"22:00"});
							$("#s\\.business").val("08:00--22:00");
							...
							var selRowIdx = jQuery(grid_selector).jqGrid("getGridParam","selrow");
							var rowData = jQuery(grid_selector).jqGrid("getRowData", selRowIdx);
							var serviceDS = new DataSourceTree({ssId:-1,ctx:"<%=ctx%>"});
							$("#serviceTree").ace_tree({
								dataSource: serviceDS,
								multiSelect: true,
								loadingHTML: '<div class="tree-loading"><i class="icon-refresh icon-spin blue"></i></div>',
						...
						onclickSubmit: function(params, postdata) {
							params.url = "<%=ctx%>/admin/addShop.do";
							alert("添加成功");
						},
						afterShowForm: function(form) {
							form.css("height", "455px");
				...
				function show_icon(form) {
					...
					form.find("#tr_icon td:eq(1) input:eq(0)").attr("name", "file");
					...
					form.find("#tr_icon td:eq(1) input:eq(0)").change(function(e) {
			            ...
			            ajax_upload_icon("icon", "serviceshop_image", this,form);
					});
					...
					form.find("#licensePic").change(function(e) {
			            ...
			            reader.readAsDataURL(file)
			            ajax_upload_licensePic("licensePic", "serviceshop_image", this,form)
					});
					if (tmp == null || tmp[1] == "") {
						src = "../assets/images/add_pic.png";
					} else {
				...
				function show_thumbnails(form) {
					...
					jQuery("#licensePic").attr("name","file");
