...
<%
	...
	// List<Order> order_list = orderDao.findOrderByUser(user.getId());
	List<Order> order_list = orderDao.findOrderByUserAndNum(user.getId(),10,0);
	int total_order = orderDao.findOrderByUserAndNum(user.getId(),0,0).size();
	int pagenum = 0;
	if(total_order % 10 == 0){
		...
		<div id="bodySection" style="overflow: hidden;" class="uc_content">
			...
			<div class="r_content">
				...
				<div class="order_data">
					<div class="order_tab">
						<a class="act_tab" href="<%=ctx%>/profile.jsp">全部订单</a>
						<a href="<%=ctx%>/Obligation2.jsp">待付款</a>
						...
						<tbody>
							...
							<tr>
								...
								<td align="center">
									...
									<%}%>
									<%if(order_list.get(i).getState() == 1){%>
									<a href="javascript:void(0)" onclick="checkuser(this)">确认使用</a>
									<a href="javascript:void(0)" onclick="changeorder(this)">退款</a>
									<%}%>
									<%if(order_list.get(i).getState() == 2){%>
				...
				<div class="page_count" style="position: relative;">
...
function checkuser(obj){
	var serialnumber = $(obj).parent().find("#serialnumber").val();
    var sp_id = $(obj).parent().find("#sp_id").val();
    var uri = "<%=ctx%>/former/changeorderstate?state=2&serialnumber=" + serialnumber + "&sp_id=" + sp_id;
    $.ajax({
        url: uri,
        type: "post",
        dataType: "JSON",
        ...
        success: function(ret){
            if(ret.change == "ok"){
                sAlert3("确认成功",window.location.href);
            }
        },
        error: function(XMLRequest, textInfo){
            if(textInfo != null){
...
function changeorder(obj){
	var serialnumber = $(obj).parent().find("#serialnumber").val();
    var sp_id = $(obj).parent().find("#sp_id").val();
    var uri = "<%=ctx%>/former/changeorderstate?state=4&serialnumber=" + serialnumber + "&sp_id=" + sp_id;
    $.ajax({
        url: uri,
        type: "post",
        dataType: "JSON",
        ...
        success: function(ret){
            if(ret.change == "ok"){
                sAlert3("申请退款中!",window.location.href);
            }
        },
        error: function(XMLRequest, textInfo){
            if(textInfo != null){
...
function commentorder(obj){
    ...
    var uri = "<%=ctx%>/former/commentorder?serialnumber=" + serialnumber + "&sp_id=" + sp_id;
    // $.ajax({
    //     url: uri,
    //     type: "post",
    //     dataType: "JSON",
...
$('.page_count a').click(function(){
	...
	start = (page - 1) * 10;
	$.ajax({
        url: "<%=ctx%>/former/getorderbypage?uid=<%=user.getId()%>&start=" + start + "&number=10&state=10",
        type: "post",
        dataType: "JSON",
        async: false,
        timeout: 3000,
