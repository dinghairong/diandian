...
<body style="min-width: 1280px;">
	<div class="all" id="all">
		...
		<a class="qyyh_login" href="<%=ctx%>/slogin.jsp"><span>登录</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;|</a>
		<a class="qyyh_login" href="<%=ctx%>/sregister.jsp">注册</a>
		<!-- <div class="top_line"></div> -->
		<div class="login_div" style="margin: 61px 226px 0 0;">
			<form id="member_login_form" onsubmit="return validateLogin();" action="<%=request.getContextPath()%>/former/slogin.do" method="post" style="box-sizing: unset;">
				<div class="login-form-row" style="margin-top: 26px;">
					<div class="title">账号：</div>
					<div class="input">
						<input id="login_name_input" class="pub-input" type="text" name="saccount" placeholder="请输入您的注册手机号码">
					</div>
				</div>
				...
				<div class="login-form-row">
					...
					<div class="input">
						<input id="login_password_input" class="pub-input" type="password" name="spassword" placeholder="请输入您的登录密码">
					</div>
				</div>
                ...
                <div class="login-form-row forget">
					<div class="title"></div>
					<div class="input">
						<div class="title" style="margin-left:20px;">还未注册?</div>
						<a class="forget_pswa" href="<%=ctx%>/sregister.jsp">免费注册</a>
					</div>
				</div>
                <!-- <div class="help_login">
                	<a class="fr" href="<%=ctx%>/stores_register.jsp">免费注册</a>
					<span class="ri">还未注册? </span>
                </div> -->
			</form>

			<img src="<%=ctx%>/images/slogin_new.png" 
...
function PassWordRecv(){
	...
	if(phone.length == 0){
		...
		$.ajax({
			url: "<%=ctx%>/former/checkshopphone.do?phone=" + phone + "&imagecode=" + imgcode,
			type:"POST",
			dataType: "JSON",
			async: false,
			timeout: 3000,
...
function saveChangePwd(){
	...
	if(new_pwd != check_pwd){
		...
		$('#errormsg3').hide();
		$.ajax({
			url: "<%=ctx%>/former/saveshopchangepwd.do?newpwd=" + new_pwd,
			type:"POST",
			dataType: "JSON",
			async: false,
			timeout: 3000,
...
function loginHX(){
	$.ajax({
		url: "<%=request.getContextPath()%>/former/slogin.do",
		type: "post",
		dataType: "JSON",
